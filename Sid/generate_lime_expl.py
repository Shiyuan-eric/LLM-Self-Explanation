...
class LimeExplanationGenerator():
    ...
    # Generates GPT response by querying OpenAI API
    ...
    def predict_proba(self, sentences):
        ...
        for i in range(len(sentences)):
            messages.append(
                {"role": "user", "content": PRE_PHRASE + sentences[i] + POST_PHRASE})
            (pred, conf, expl) = self.parse_completion(
                self.generate_response().choices[0].message.content)
            messages.pop()
            probs[i, pred] = conf
...
if __name__ == "__main__":
    ...
    batch_size = 5  # checkpoints every batch_size batches into a pickle file
    for i in tqdm(range(start, end, batch_size)):
        lime_generator = LimeExplanationGenerator(
            'sentences.pickle', PE=False, messages=messages, start=i, end=i+batch_size)
    ...
    prefixed = [filename for filename in os.listdir(
        '.') if filename.startswith("LIME_response_EP_")]
    explanations = []
